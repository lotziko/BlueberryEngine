...
local function addCompilerProps(cfg)
  ...
  -- Custom compile path and type for ptx
  if cfg.cudaPtxDir ~= nil and cfg.cudaPtxDir ~= '' then
    local e = { }
    e.cfg = cfg
	e.prj = cfg.project
	e.wks = cfg.project.workspace
    local v = premake.detoken.expand(cfg.cudaPtxDir, e)
	local rel = path.getrelative(cfg.project.location, v)
    writeString('CompileOut', path.translate(rel .. "/%%(Filename).ptx", "\\"))
	writeString('NvccCompilation', "ptx")
  end
