...
namespace Blueberry
{
	void RendererTree::Add(ObjectId id, const AABB& bounds)
	{
		m_Octree.Add(bounds, id);
	}

	void RendererTree::Remove(ObjectId id, const AABB& bounds)
	{
		m_Octree.Remove(bounds, id);
	}

	void RendererTree::Update(ObjectId id, const AABB& previousBounds, const AABB& newBounds)
	{
