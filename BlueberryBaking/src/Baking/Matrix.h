...
template <unsigned int M> inline __device__ Matrix<M, M>& operator*=(Matrix<M, M>& m1, const Matrix<M, M>& m2);
RT_MAT_DECL inline __device__ bool         operator==(const Matrix<M, N>& m1, const Matrix<M, N>& m2);
RT_MAT_DECL inline __device__ bool         operator!=(const Matrix<M, N>& m1, const Matrix<M, N>& m2);
RT_MAT_DECL inline __device__ Matrix<M, N>& operator-=(Matrix<M, N>& m1, const Matrix<M, N>& m2);
RT_MAT_DECL inline __device__ Matrix<M, N>& operator+=(Matrix<M, N>& m1, const Matrix<M, N>& m2);
RT_MAT_DECL inline __device__ Matrix<M, N>& operator*=(Matrix<M, N>& m1, float f);
RT_MAT_DECL inline __device__ Matrix<M, N>&operator/=(Matrix<M, N>& m1, float f);
RT_MAT_DECL inline __device__ Matrix<M, N> operator-(const Matrix<M, N>& m1, const Matrix<M, N>& m2);
RT_MAT_DECL inline __device__ Matrix<M, N> operator+(const Matrix<M, N>& m1, const Matrix<M, N>& m2);
RT_MAT_DECL inline __device__ Matrix<M, N> operator/(const Matrix<M, N>& m, float f);
RT_MAT_DECL inline __device__ Matrix<M, N> operator*(const Matrix<M, N>& m, float f);
RT_MAT_DECL inline __device__ Matrix<M, N> operator*(float f, const Matrix<M, N>& m);
RT_MAT_DECL inline __device__ typename Matrix<M, N>::floatM operator*(const Matrix<M, N>& m, const typename Matrix<M, N>::floatN& v);
RT_MAT_DECL inline __device__ typename Matrix<M, N>::floatN operator*(const typename Matrix<M, N>::floatM& v, const Matrix<M, N>& m);
template<unsigned int M, unsigned int N, unsigned int R> inline __device__ Matrix<M, R> operator*(const Matrix<M, N>& m1, const Matrix<N, R>& m2);


// Partial specializations to make matrix vector multiplication more efficient
template <unsigned int N>
inline __device__ float2 operator*(const Matrix<2, N>& m, const typename Matrix<2, N>::floatN& vec);
template <unsigned int N>
inline __device__ float3 operator*(const Matrix<3, N>& m, const typename Matrix<3, N>::floatN& vec);
inline __device__ float3 operator*(const Matrix<3, 4>& m, const float4& vec);
template <unsigned int N>
inline __device__ float4 operator*(const Matrix<4, N>& m, const typename Matrix<4, N>::floatN& vec);
inline __device__ float4 operator*(const Matrix<4, 4>& m, const float4& vec);
...
public:
	...
	__device__  explicit     Matrix(const float data[M*N]) { for (unsigned int i = 0; i < M*N; ++i) m_data[i] = data[i]; }

	/** Copy the matrix */
	__device__               Matrix(const Matrix& m);

	__device__               Matrix(const std::initializer_list<float>& list);

	/** Assignment operator */
	__device__  Matrix&      operator=(const Matrix& b);

	/** Access the specified element 0..N*M-1  */
	__device__  float        operator[](unsigned int i)const { return m_data[i]; }

	/** Access the specified element 0..N*M-1  */
	__device__  float&       operator[](unsigned int i) { return m_data[i]; }

	/** Access the specified row 0..M.  Returns float, float2, float3 or float4 depending on the matrix size  */
	__device__  floatN       getRow(unsigned int m)const;

	/** Access the specified column 0..N.  Returns float, float2, float3 or float4 depending on the matrix size */
	__device__  floatM       getCol(unsigned int n)const;

	/** Returns a pointer to the internal data array.  The data array is stored in row-major order. */
	__device__  float*       getData();

	/** Returns a const pointer to the internal data array.  The data array is stored in row-major order. */
	__device__  const float* getData()const;

	/** Assign the specified row 0..M.  Takes a float, float2, float3 or float4 depending on the matrix size */
	__device__  void         setRow(unsigned int m, const floatN &r);

	/** Assign the specified column 0..N.  Takes a float, float2, float3 or float4 depending on the matrix size */
	__device__  void         setCol(unsigned int n, const floatM &c);

	/** Returns the transpose of the matrix */
	__device__  Matrix<N, M>         transpose() const;

	/** Returns the inverse of the matrix */
	__device__  Matrix<4, 4>         inverse() const;

	/** Returns the determinant of the matrix */
	__device__  float               det() const;

	/** Returns a rotation matrix */
	__device__  static Matrix<4, 4>  rotate(const float radians, const float3& axis);

	/** Returns a translation matrix */
	__device__  static Matrix<4, 4>  translate(const float3& vec);

	/** Returns a scale matrix */
	__device__  static Matrix<4, 4>  scale(const float3& vec);

	/** Creates a matrix from an ONB and center point */
	__device__  static Matrix<4, 4>  fromBasis(const float3& u, const float3& v, const float3& w, const float3& c);

	/** Returns the identity matrix */
	__device__  static Matrix<3, 4>  affineIdentity();
	__device__  static Matrix<N, N>  identity();

	/** Ordered comparison operator so that the matrix can be used in an STL container */
	__device__  bool         operator<(const Matrix<M, N>& rhs) const;
...
template<unsigned int M, unsigned int N>
inline __device__ Matrix<M, N>::Matrix()
{
}


template<unsigned int M, unsigned int N>
inline __device__ Matrix<M, N>::Matrix(const Matrix<M, N>& m)
{
...
template<unsigned int M, unsigned int N>
inline __device__ Matrix<M, N>::Matrix(const std::initializer_list<float>& list)
{
	int i = 0;
...
template<unsigned int M, unsigned int N>
inline __device__ Matrix<M, N>&  Matrix<M, N>::operator=(const Matrix& b)
{
	for (unsigned int i = 0; i < M*N; ++i)
		m_data[i] = b[i];
	return *this;
...
/*
template<unsigned int M, unsigned int N>
inline __device__ float Matrix<M,N>::operator[]( unsigned int i )const
{
return m_data[i];
}
...
template<unsigned int M, unsigned int N>
inline __device__ float& Matrix<M,N>::operator[]( unsigned int i )
{
return m_data[i];
}
*/
...
template<unsigned int M, unsigned int N>
inline __device__ typename Matrix<M, N>::floatN Matrix<M, N>::getRow(unsigned int m)const
{
...
template<unsigned int M, unsigned int N>
inline __device__ typename Matrix<M, N>::floatM Matrix<M, N>::getCol(unsigned int n)const
{
...
template<unsigned int M, unsigned int N>
inline __device__ float* Matrix<M, N>::getData()
{
	return m_data;
}
...
template<unsigned int M, unsigned int N>
inline __device__ const float* Matrix<M, N>::getData() const
{
	return m_data;
}
...
template<unsigned int M, unsigned int N>
inline __device__ void Matrix<M, N>::setRow(unsigned int m, const typename Matrix<M, N>::floatN &r)
{
...
template<unsigned int M, unsigned int N>
inline __device__ void Matrix<M, N>::setCol(unsigned int n, const typename Matrix<M, N>::floatM &c)
{
...
template<unsigned int M, unsigned int N>
__device__  bool operator==(const Matrix<M, N>& m1, const Matrix<M, N>& m2)
{
	for (unsigned int i = 0; i < M*N; ++i)
...
template<unsigned int M, unsigned int N>
__device__  Matrix<M, N> operator-(const Matrix<M, N>& m1, const Matrix<M, N>& m2)
{
	Matrix<M, N> temp(m1);
	temp -= m2;
	return temp;
...
// Subtract two matrices of the same size.
template<unsigned int M, unsigned int N>
__device__  Matrix<M, N>& operator-=(Matrix<M, N>& m1, const Matrix<M, N>& m2)
{
	for (unsigned int i = 0; i < M*N; ++i)
		m1[i] -= m2[i];
	return m1;
...
__device__  Matrix<M, R> operator*(const Matrix<M, N>& m1, const Matrix<N, R>& m2)
{
	Matrix<M, R> temp;
...
__device__  Matrix<M, M>& operator*=(Matrix<M, M>& m1, const Matrix<M, M>& m2)
{
	m1 = m1 * m2;
	return m1;
}
...
template<unsigned int M, unsigned int N>
__device__  typename Matrix<M, N>::floatM operator*(const Matrix<M, N>& m, const typename Matrix<M, N>::floatN& vec)
{
	typename Matrix<M, N>::floatM temp;
...
template<unsigned int M, unsigned int N>
__device__  typename Matrix<M, N>::floatN operator*(const typename Matrix<M, N>::floatM& vec, const Matrix<M, N>& m)
{
	typename Matrix<M, N>::floatN  temp;
	float* t = reinterpret_cast<float*>(&temp);
	const float* v = reinterpret_cast<const float*>(&vec);
...
template<unsigned int M, unsigned int N>
__device__  Matrix<M, N> operator*(const Matrix<M, N>& m, float f)
{
	Matrix<M, N> temp(m);
	temp *= f;
	return temp;
...
// Multply matrix by a scalar.
template<unsigned int M, unsigned int N>
__device__  Matrix<M, N>& operator*=(Matrix<M, N>& m, float f)
{
	for (unsigned int i = 0; i < M*N; ++i)
		m[i] *= f;
	return m;
...
// Multply matrix by a scalar.
template<unsigned int M, unsigned int N>
__device__  Matrix<M, N>  operator*(float f, const Matrix<M, N>& m)
{
	Matrix<M, N> temp;
...
// Returns the transpose of the matrix.
template<unsigned int M, unsigned int N>
inline __device__ Matrix<N, M> Matrix<M, N>::transpose() const
{
	Matrix<N, M> ret;
...
// Returns the determinant of the matrix.
template<>
inline __device__ float Matrix<3, 3>::det() const
{
...
// Returns the determinant of the matrix.
template<>
inline __device__ float Matrix<4, 4>::det() const
{
	const float* m = m_data;
	float d =
		m[0] * m[5] * m[10] * m[15] -
...
// Returns the inverse of the matrix.
template<>
inline __device__ Matrix<4, 4> Matrix<4, 4>::inverse() const
{
	Matrix<4, 4> dst;
	const float* m = m_data;
...
template<>
inline __device__ Matrix<4, 4> Matrix<4, 4>::rotate(const float radians, const float3& axis)
{
...
// This is a static member.
template<>
inline __device__ Matrix<4, 4> Matrix<4, 4>::translate(const float3& vec)
{
	Matrix<4, 4> Mat = Matrix<4, 4>::identity();
	float *m = Mat.getData();
...
// This is a static member.
template<>
inline __device__ Matrix<4, 4> Matrix<4, 4>::scale(const float3& vec)
{
	Matrix<4, 4> Mat = Matrix<4, 4>::identity();
	float *m = Mat.getData();
...
// This is a static member.
template<>
inline __device__ Matrix<4, 4>  Matrix<4, 4>::fromBasis(const float3& u, const float3& v, const float3& w, const float3& c)
{
	float m[16];
	m[0] = u.x;
	m[1] = v.x;
...
template<>
inline __device__ Matrix<3, 4> Matrix<3, 4>::affineIdentity()
{
	Matrix<3, 4> m;
	m.m_data[0] = 1.0f;
	m.m_data[1] = 0.0f;
...
// This is a static member.
template<unsigned int M, unsigned int N>
inline __device__ Matrix<N, N> Matrix<M, N>::identity()
{
	float temp[N*N];
...
// Ordered comparison operator so that the matrix can be used in an STL container.
template<unsigned int M, unsigned int N>
inline __device__ bool Matrix<M, N>::operator<(const Matrix<M, N>& rhs) const
{
...
inline __device__ Matrix<3, 3> make_matrix3x3(const Matrix<4, 4> &matrix)
{
	Matrix<3, 3> Mat;
	float *m = Mat.getData();
