...
namespace Blueberry
{
	Ray SceneHelper::GUIPointToWorldRay(const Vector2& position, Camera* camera)
	{
		...
		Vector3 rayPointCameraSpace = Math::MultiplyPoint(clipToCamera, rayPointClipSpace);

		if (camera->IsOrthographic())
		{
			Vector3 rayDirectionCameraSpace = Vector3(0.0f, 0.0f, -1.0f);
			rayDirectionWorldSpace = Math::MultiplyVector(cameraToWorld, rayDirectionCameraSpace);
			...
			rayOriginWorldSpace = Math::MultiplyPoint(cameraToWorld, rayOriginCameraSpace);
		}
		else
		{
			...
			Vector3 cameraPositionWorldSpace = Math::MultiplyPoint(cameraToWorld, Vector3(0, 0, 0));
			// Do not invert startZ because of right handed coordinate system
