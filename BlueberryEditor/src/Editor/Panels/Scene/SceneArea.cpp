...
namespace Blueberry
{
	...
	void SceneArea::OnDrawUI()
	{
		...
		bool popupOpen = ImGui::IsPopupOpen(NULL, ImGuiPopupFlags_AnyPopupId);

		// Motion
		if (!popupOpen && ImGui::IsWindowFocused())
		{
		...
		if (!popupOpen && mouseWheelDelta != 0)
		{
		...
		// Dragging
		if (!popupOpen && ImGui::IsMouseDragging(1, 0))
		{
